...
export default function MyBooking() {
    ...
    const getUserDetails = async () => {
      try {
        const response = await axiosInstance.get(
          `${API_BASE_URL}/api/method/airport_transport.api.bookings.my_bookings?language=${
        ...
        if (response && response.status === 200) {
          ...
          // if(data.length > 0){
          const totalLength = response.data.data.length;
  ...
  return (
    <div>
      ...
      <div
        className="pt-1 md:pt-20 lg:pt-20"
      ...
      >
        <div className="md:px-14 py-7 border-t border-text_white bg-background_steel_blue">
          ...
          <div className="flex flex-row justify-center items-center w-full md:w-1/2 md:justify-end mt-3">
              ...
              <ul className="flex items-center -space-x-px h-10 text-base">
                ...
                {totalPages.map((data, index) => (
                  <li onClick={() => setCurrentPage(data.id)}>
                    <a
                      className={`w-[30px] flex items-center justify-center cursor-pointer md:px-4 px-1 h-10 leading-tight ${
                        currentPage === data.id
