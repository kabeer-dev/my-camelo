  ...
  const validationSchema = Yup.object().shape({
    ...
    confirmNewPassword: Yup.string()
      .oneOf(
        ...
        t("errors.confirm_password_error2")
      )
  ...
  const onSubmit = async (values, { setSubmitting }) => {
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        dispatch(
          signInRequest(email, values.newPassword, recaptchaToken, navigate)
        );
        ...
        <div className="flex flex-col justify-center items-center">
          ...
          <div className="block w-72 md:w-auto p-5 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
            <div dir={language === "ar" ? "rtl" : "ltr"}>
              <div className="w-auto md:w-[400px] text-left rtl:text-right">
                <Heading
                  title={t("reset_password_text")}
                  className={
                    "text-2xl text-[#0E0E11] text-center md:text-left md:rtl:text-right"
                  }
                />
              </div>
              <div className="text-sm text-text_lightdark_grey text-center md:text-left md:rtl:text-right">
