  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        navigate("/mashrouk-new-ui/agent/reset-password", { state: { otp: otp, email: email } });
      }
    } catch (error) {
  ...
  const handleResend = async () => {
    ...
    try {
      const otpResponse = await axiosInstance.get(
        `${API_BASE_URL}/api/method/airport_transport.api.user.reset_password?email=${email}`,
      );
                ...
                <div className="w-auto md:w-3/4">
                  <p className="text-sm text-[#5D5D5D]">
                    ...
                    <span
                      ...
                      onClick={() => navigate("/mashrouk-new-ui/agent/forget-password")}
                    >
                      ({t("change_text")})
                    </span>
                  </p>
