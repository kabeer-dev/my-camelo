  ...
  return (
    <ol className={className} style={{ borderColor: "lightgray" }}>
      {steps.map((step, index) => (
        <li
          key={index}
          className={`flex items-center ${index === activeStepIndex ? "text-blue-600" : "text-gray-500"
            } ${index === steps.length - 1 ? "justify-end" : "justify-between"}`}
          style={{ width: stepWidth }}
        >
          <div className="flex flex-col md:flex-row items-center pr-2">
            <span
              className={`flex items-center justify-center w-5 h-5 me-2 text-xs rounded-full shrink-0 ${index === activeStepIndex
